...
export default function SSMExponentialModeViz() {
  ...
  return (
    <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-4 dark:border-indigo-800 dark:bg-indigo-950/20">
      <p className="mb-2 text-sm font-bold uppercase tracking-wide text-indigo-700 dark:text-indigo-300">
        인터랙티브 ❷ — SSM = 지수 모드의 합 (다항 포락선 근사)
      </p>
      <p className="mb-3 text-sm leading-relaxed text-indigo-700 dark:text-indigo-400">
      ...
      <div className="mb-3 flex items-center gap-3">
        <span className="w-28 shrink-0 text-sm font-semibold text-indigo-600 dark:text-indigo-400">지수 모드 수 M</span>
        <input
          type="range"
          min={1}
          max={8}
        ...
        />
        <span className="w-8 shrink-0 text-right font-mono text-sm font-bold text-indigo-700 dark:text-indigo-300">
          {numModes}
        </span>
      </div>
      ...
      >
        {/* Background */}
        <rect x={0} y={0} width={chartW + padL + 20} height={chartH + padT + 30} fill="#f8fafc" />
        ...
        {/* Axes */}
        <line x1={padL} y1={padT} x2={padL} y2={padT + chartH} stroke="#9ca3af" strokeWidth={0.8} />
        <line x1={padL} y1={padT + chartH} x2={padL + chartW} y2={padT + chartH} stroke="#9ca3af" strokeWidth={0.8} />
        <text x={padL + chartW + 10} y={padT + chartH + 3} fontSize={10} fill="#9ca3af">
          d
        </text>
        <text x={padL + 3} y={padT + 5} fontSize={10} fill="#9ca3af">
          크기
        </text>

        {/* y-axis labels */}
        {[0.25, 0.5, 1.0].map((v) => (
          <text key={v} x={padL - 4} y={vy(v) + 3} textAnchor="end" fontSize={10} fill="#9ca3af">
            {v}
          </text>
        ))}
        ...
        {[0, 4, 8, 12, 16, 20, 24].map((d) => (
          <text key={d} x={vx(d)} y={padT + chartH + 14} textAnchor="middle" fontSize={10} fill="#9ca3af">
            {d}
          </text>
        ))}
        ...
        {(() => {
          ...
          return <polyline points={pts} fill="none" stroke="#f97316" strokeWidth={2.5} />;
        })()}

        {/* Sum approximation */}
        {(() => {
          const pts = rows.map(({ d, approx }) => `${vx(d)},${vy(approx)}`).join(' ');
          return <polyline points={pts} fill="none" stroke="#6366f1" strokeWidth={2.5} />;
        })()}

        {/* Legend */}
        <line x1={padL + 10} y1={padT + chartH - 30} x2={padL + 28} y2={padT + chartH - 30} stroke="#f97316" strokeWidth={2.5} />
        <text x={padL + 32} y={padT + chartH - 26} fontSize={10} fill="#9ca3af">
          목표: 1/(1+d)
        </text>
        <line x1={padL + 10} y1={padT + chartH - 14} x2={padL + 28} y2={padT + chartH - 14} stroke="#6366f1" strokeWidth={2.5} />
        <text x={padL + 32} y={padT + chartH - 10} fontSize={10} fill="#9ca3af">
          SSM 합 (M={numModes}개 모드)
        </text>
      </svg>

      <p className="mt-1 text-sm italic text-indigo-600 dark:text-indigo-400">
