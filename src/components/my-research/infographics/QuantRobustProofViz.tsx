...
const PROOF_STEPS = [
  {
    ...
    body: (
      <>
        <p className="mb-2 text-xs text-slate-700 dark:text-slate-300">상태 오차를 정의합니다:</p>
        <BlockMath latex={String.raw`e_t \triangleq \|\mathbf{s}_t - \hat{\mathbf{s}}_t\|`} />
        <p className="text-xs text-slate-600 dark:text-slate-400">초기 조건 <InlineMath latex="e_0 = 0" />이 주어집니다 (동일한 초기화 가정).</p>
      </>
    ),
  },
  {
    ...
    <div className="mt-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 p-4">
      ...
      <div className="space-y-2 mb-3">
        <div className="flex items-center gap-3 text-xs">
          <label className="w-32 flex-shrink-0 text-slate-600 dark:text-slate-300">
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
      ...
      <div className="mt-3 flex items-center justify-between">
        <button
          ...
          disabled={step === 0}
          className="rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-30 transition"
        >
          ← 이전
        </button>
        <span className="text-xs text-slate-400">{step + 1} / {PROOF_STEPS.length}</span>
        <button
