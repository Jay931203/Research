...
export default function DirichletKernelViz() {
  ...
  return (
    <div className="rounded-xl border border-orange-200 bg-orange-50 p-4 dark:border-orange-800 dark:bg-orange-950/20">
      <p className="mb-2 text-xs font-bold uppercase tracking-wide text-orange-700 dark:text-orange-300">
        인터랙티브 ❶ — Dirichlet 커널 & Off-grid 스펙트럼 누설
      </p>
      <p className="mb-3 text-xs leading-relaxed text-orange-700 dark:text-orange-400">
      ...
      <div className="mb-3 flex items-center gap-3">
        <span className="shrink-0 text-xs text-orange-600 dark:text-orange-400">off-grid δ</span>
        <input
          type="range"
          min={-0.49}
          max={0.49}
        ...
        />
        <span className="w-16 shrink-0 text-right font-mono text-xs font-bold text-orange-700 dark:text-orange-300">
          δ = {delta.toFixed(2)}
        </span>
      </div>
      ...
      >
        {/* Axes */}
        <line x1={padL} y1={chartH + 2} x2={chartW + padL} y2={chartH + 2} stroke="#9ca3af" strokeWidth={0.5} />
        <text x={chartW + padL + 18} y={chartH + 5} fontSize={6.5} fill="#9ca3af" textAnchor="end">
          d
        </text>
        <text x={padL + 2} y={8} fontSize={6.5} fill="#9ca3af">
          |X[d]|
        </text>
        ...
        {bins.map(({ d, mag }) => {
          ...
          return (
            <g key={d}>
              ...
              {/* x label every 4 */}
              {d % 4 === 0 && (
                <text x={cx} y={chartH + 13} textAnchor="middle" fontSize={6} fill="#9ca3af">
                  {d}
                </text>
              )}
            </g>
        ...
        {(() => {
          const pts = bins
            ...
            .join(' ');
          return (
            <polyline points={pts} fill="none" stroke="#6366f1" strokeWidth={1.5} strokeDasharray="3,2" />
          );
        })()}

        {/* Legend */}
        <rect x={padL + 10} y={chartH - 22} width={10} height={7} fill="#fb923c" rx={1} />
        <text x={padL + 24} y={chartH - 16} fontSize={6.5} fill="#9ca3af">
          DFT 크기 |X[d]|
        </text>
        <line
          x1={padL + 10}
          ...
          strokeDasharray="3,2"
        />
        <text x={padL + 24} y={chartH - 4} fontSize={6.5} fill="#9ca3af">
          포락선 1/(1+|d|)
        </text>
      </svg>
